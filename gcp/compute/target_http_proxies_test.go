...
var _ = Describe("TargetHttpProxies", func() {
	...
	Describe("List", func() {
		...
		BeforeEach(func() {
			...
			client.ListTargetHttpProxiesCall.Returns.TargetHttpProxyList = &gcpcompute.TargetHttpProxyList{
		...
		It("lists, filters, and prompts for target http proxies to delete", func() {
			list, err := targetHttpProxies.List(filter, false)
			Expect(err).NotTo(HaveOccurred())

			Expect(client.ListTargetHttpProxiesCall.CallCount).To(Equal(1))

			Expect(logger.PromptWithDetailsCall.CallCount).To(Equal(1))
			Expect(logger.PromptWithDetailsCall.Receives.ResourceType).To(Equal("Target Http Proxy"))
			Expect(logger.PromptWithDetailsCall.Receives.ResourceName).To(Equal("banana-target-http-proxy"))
		...
		Context("when the client fails to list target http proxies", func() {
			...
			It("returns the error", func() {
				_, err := targetHttpProxies.List(filter, false)
		...
		Context("when the target http proxy name does not contain the filter", func() {
			It("does not add it to the list", func() {
				list, err := targetHttpProxies.List("grape", false)
				Expect(err).NotTo(HaveOccurred())
