	...
	Describe("List", func() {
		...
		Context("when the client fails to describe a key", func() {
			...
			It("logs the error", func() {
				...
				Expect(logger.PrintfCall.Receives.Message).To(ContainSubstring("ERROR describing key"))
				Expect(logger.PrintfCall.Receives.Arguments[0]).To(Equal("banana"))
		...
		Context("when the client fails to list resource tags", func() {
			...
			It("logs the error", func() {
				...
				Expect(logger.PrintfCall.CallCount).To(Equal(1))
				Expect(logger.PrintfCall.Receives.Message).To(ContainSubstring("ERROR listing tags for key"))
				Expect(logger.PrintfCall.Receives.Arguments[0]).To(Equal("banana"))
				Expect(logger.PrintfCall.Receives.Arguments[1]).To(MatchError("some error"))
			})
		})
