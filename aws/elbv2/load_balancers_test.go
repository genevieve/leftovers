...
var _ = Describe("LoadBalancers", func() {
	...
	Describe("List", func() {
		...
		It("returns a list of elbv2 load balancers to delete", func() {
			...
			Expect(logger.PromptWithDetailsCall.Receives.Type).To(Equal("Elastic Load Balancer V2"))
		...
		Context("when the client fails to list load balancers", func() {
			...
			It("returns the error", func() {
				...
				Expect(err).To(MatchError("Describing Elastic Load Balancers V2: some error"))
			})
		})
